...
plot_retreat_curve <- function(ombc_out) {
  ...
  max_value <- ombc_out$retreat_vals[1]
  max_step <- ombc_out$retreat_vals[2]

  outlier_seq <- seq(gross_num, max_out)
  point_size <- 1 - min(0.9, max(0, -0.1 + max_out / 250))

  retreat <- minimum <- choice <- NULL
  retreat_curve_df <- data.frame(
    "outlier_seq" = outlier_seq,
    "minimum" = as.integer(outlier_num["full"]),
    "choice" = as.integer(outlier_num["retreat"]),
    "retreat" = distrib_diff_mat[, "full"] / min(distrib_diff_mat[, "full"])
  )
  retreat_curve <- retreat_curve_df |>
    ggplot2::ggplot(ggplot2::aes(x = outlier_seq, y = retreat)) +
    ggplot2::geom_line(ggplot2::aes(colour = "retreat"), show.legend = FALSE) +
    ggplot2::geom_point(
    ...
    ggplot2::geom_hline(
      ggplot2::aes(
        yintercept = min(retreat) * (1 + max_value), colour = "minimum"
      ),
      linetype = "dashed", linewidth = 0.75, show.legend = FALSE
    ) +
    ggplot2::scale_colour_manual(
      values = c(retreat = "#000000", choice = "#CC79A7", minimum = "#009E73")
    ) +
    ggplot2::labs(
      title =
