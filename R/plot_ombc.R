...
#' @returns A gg object.
#' @export
plot_ks_curve <- function(ombc_out, alpha = 0.05) {
  gross_num <- sum(ombc_out$gross_outs)
  max_out <- max(ombc_out$outlier_rank) - 1
  outlier_num <- ombc_out$outlier_num
  distrib_diff_mat <- ombc_out$distrib_diff_mat
  ...
  ks_curve_df <- data.frame(
    ...
    "ks" = distrib_diff_mat[, "ks"]
  )
  ks_curve <- ks_curve_df |>
    ggplot2::ggplot(ggplot2::aes(x = outlier_seq, y = ks)) +
    ggplot2::geom_line(
      ggplot2::aes(colour = "ks"),
      show.legend = FALSE
    ) +
    ggplot2::geom_point(
      ggplot2::aes(colour = "ks"),
    ...
    ggplot2::geom_line(
      ggplot2::aes(y = threshold, colour = "threshold"),
      linetype = "solid", linewidth = 0.75, show.legend = FALSE
    ) +
    ggplot2::scale_colour_manual(
      values = c(ks = "#000000", minimum = "#CC79A7", threshold = "#009E73")
    ) +
    ggplot2::labs(
      title = paste0("outlierMBC: Number of Outliers = ", outlier_num["ks"]),
      x = "Outlier Number",
